 ...
 * {@link Rest5Client}. Must be created via {@link SnifferBuilder}, which allows to set all of the different options or rely on defaults.
 * A background task fetches the nodes through the {@link NodesSniffer} and sets them to the {@link Rest5Client} instance.
 ...
 * {@link Rest5ClientBuilder#setFailureListener(Rest5Client.FailureListener)}. The Sniffer implementation needs to be lazily set to the
...
public class Sniffer implements Closeable {
     ...
     *      {@link Rest5Client#setNodes(Collection)}) once they are fetched

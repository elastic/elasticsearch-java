...
public class DeadHostStateTests extends RestClientTestCase {
    ...
    public void testCompareToTimeSupplier() {
        ...
        for (int i = 0; i < numObjects; i++) {
            if (i == 0) {
                // this test requires a strictly increasing timer. This ensures that even if we call this
                // time supplier in a very tight
                // loop we always notice time moving forward. This does not happen for real timer
                // implementations
        ...
        for (int k = 1; k < deadHostStates.length; k++) {
            assertThat(deadHostStates[k - 1].getDeadUntilNanos(),
                lessThan(deadHostStates[k].getDeadUntilNanos()));
    ...
    public void testCompareToDifferingTimeSupplier() {
        try {
            ...
            assertEquals(
                "can't compare DeadHostStates holding different time suppliers as they may be based on " +
                    "different clocks",
                e.getMessage()
            );
        }
    ...
    public void testDeadHostStateTimeouts() {
        ...
        for (long expectedTimeoutsSecond : EXPECTED_TIMEOUTS_SECONDS) {
            assertThat(TimeUnit.NANOSECONDS.toSeconds(previous.getDeadUntilNanos()),
                equalTo(expectedTimeoutsSecond));

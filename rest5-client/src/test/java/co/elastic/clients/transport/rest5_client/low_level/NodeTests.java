...
public class NodeTests extends RestClientTestCase {
    ...
    public void testToString() {
        ...
        assertEquals(
            "[host=http://1, attributes={foo=[bar], baz=[bort, zoom]}]",
            new Node(new HttpHost("1"), null, null, null, null, attributes).toString()
        );
        assertEquals(
            "[host=http://1, roles=data,ingest,master]",
            new Node(new HttpHost("1"), null, null, null, new Node.Roles(new TreeSet<>(Arrays.asList("master", "data", "ingest"))), null)
                .toString()
        );
        ...
        assertEquals(
            ...
            new Node(new HttpHost("1"), new HashSet<>(Arrays.asList(new HttpHost("1"), new HttpHost("2"))), null, null, null, null)
                .toString()
        );
        assertEquals(
            "[host=http://1, bound=[http://1, http://2], "
            ...
            new Node(
                new HttpHost("1"),
                new HashSet<>(Arrays.asList(new HttpHost("1"), new HttpHost("2"))),
                "nam",
                "ver",
                new Node.Roles(Collections.singleton("master")),
                attributes
            ).toString()
        );
    }
    ...
    public void testEqualsAndHashCode() {
        ...
        Node node = new Node(
            ...
            randomBoolean() ? null : randomAsciiAlphanumOfLength(5),
            randomBoolean() ? null : new Node.Roles(new TreeSet<>(Arrays.asList("master", "data", "ingest"))),
        ...
        assertFalse(
            node.equals(
                new Node(
                    new HttpHost(host.toHostString() + "changed"),
                    node.getBoundHosts(),
                    node.getName(),
                    node.getVersion(),
                    node.getRoles(),
        ...
        assertFalse(
            node.equals(
                new Node(
                    host,
                    new HashSet<>(Arrays.asList(host, new HttpHost(host.toHostString() + "changed"))),
                    node.getName(),
                    node.getVersion(),
                    node.getRoles(),
                    node.getAttributes()
        ...
        assertFalse(
            node.equals(
                new Node(host, node.getBoundHosts(), node.getName() + "changed", node.getVersion(), node.getRoles(), node.getAttributes())
            )
        );
        assertFalse(
            node.equals(
                new Node(host, node.getBoundHosts(), node.getName(), node.getVersion() + "changed", node.getRoles(), node.getAttributes())
            )
        );
        assertFalse(
            node.equals(
                new Node(
                    ...
                    node.getName(),
                    node.getVersion(),
                    new Node.Roles(Collections.emptySet()),
                    node.getAttributes()
                )
            )
        );
        assertFalse(
            node.equals(
                new Node(
                    ...
                    node.getVersion(),
                    node.getRoles(),
                    singletonMap("bort", singletonList("bing"))
                )
            )
        );
    }
    ...
    public void testDataRole() {
        ...
        roles = new Node.Roles(new TreeSet<>(Arrays.asList("data_warm")));
        assertTrue(roles.hasDataWarmRole());
        assertTrue(roles.canContainData());
        roles = new Node.Roles(new TreeSet<>(Arrays.asList("data_cold")));
        assertTrue(roles.hasDataColdRole());
        assertTrue(roles.canContainData());
        roles = new Node.Roles(new TreeSet<>(Arrays.asList("data_frozen")));
        assertTrue(roles.hasDataFrozenRole());
        assertTrue(roles.canContainData());
        roles = new Node.Roles(new TreeSet<>(Arrays.asList("data_content")));
        assertTrue(roles.hasDataContentRole());
        assertTrue(roles.canContainData());
        roles = new Node.Roles(new TreeSet<>(Arrays.asList("data")));
        assertTrue(roles.hasDataRole());
        assertTrue(roles.canContainData());
        roles = new Node.Roles(new TreeSet<>(Arrays.asList("data_foo")));
        assertTrue(roles.canContainData());
    }
}

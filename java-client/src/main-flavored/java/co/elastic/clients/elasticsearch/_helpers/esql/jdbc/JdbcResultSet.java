...
import static co.elastic.clients.elasticsearch._helpers.esql.jdbc.EsType.DATE;
import static co.elastic.clients.elasticsearch._helpers.esql.jdbc.EsType.DATETIME;
import static co.elastic.clients.elasticsearch._helpers.esql.jdbc.EsType.TIME;
...
class JdbcResultSet implements ResultSet, JdbcWrapper {
    ...
    JdbcResultSet(Cursor cursor) {
        ...
        TimeZone cfgTimeZone = TimeZone.getDefault();
        this.defaultCalendar = Calendar.getInstance(cfgTimeZone, Locale.ROOT);
...
//            }
        ...
        try {
            ...
            if (DATETIME == type) {
                ...
                if (val instanceof String) {
                    return JdbcDateUtils.asZonedDateTime((String) val).toInstant().toEpochMilli();
                } else {
                    return ((Number) val).longValue();
                }
            }
            if (DATE == type) {
                return JdbcDateUtils.dateTimeAsMillisSinceEpoch(val.toString());
            }
            if (TIME == type) {
                return JdbcDateUtils.timeAsMillisSinceEpoch(val.toString());
            }
            return (Long) val;
        } catch (ClassCastException cce) {
    ...
    private Time asTime(int columnIndex) throws SQLException {
        ...
        try {
            if (type == TIME) {
                return JdbcDateUtils.timeAsTime(val.toString());
            }
    ...
    private Timestamp asTimeStamp(int columnIndex) throws SQLException {
        ...
        try {
            ...
            if (type == TIME) {
                return JdbcDateUtils.timeAsTimestamp(val.toString());
            }

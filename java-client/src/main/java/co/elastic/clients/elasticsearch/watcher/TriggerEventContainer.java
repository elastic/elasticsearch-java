...
public class TriggerEventContainer extends TaggedUnion<TriggerEventContainer.Tag, Object> implements ToJsonp {
	...
	private TriggerEventContainer(Builder builder) {
		super(builder.$tag, builder.$variant);

	}

	/**
	 * Is this {@link TriggerEventContainer} of a {@code schedule} kind?
	 */
	...
	public static class Builder {
		...
		public ObjectBuilder<TriggerEventContainer> schedule(ScheduleTriggerEvent v) {
			this.$variant = v;
			this.$tag = Tag.schedule;
			return new ObjectBuilder.Constant<>(this.build());
		}

		public ObjectBuilder<TriggerEventContainer> schedule(
		...
		protected TriggerEventContainer build() {
			return new TriggerEventContainer(this);
		}
	...
	public static final JsonpValueParser<TriggerEventContainer> JSONP_PARSER = JsonpValueParser
			.lazy(TriggerEventContainer::buildJsonpParser);

	private static JsonpValueParser<TriggerEventContainer> buildJsonpParser() {
		JsonpObjectParser<Builder> op = new JsonpObjectParser<>(Builder::new);

		op.add(Builder::schedule, ScheduleTriggerEvent.JSONP_PARSER, "schedule");

	...
	public static final Endpoint<SnapshotsRequest, SnapshotsResponse, ErrorResponse> _ENDPOINT = new SimpleEndpoint<>(
			...
			request -> {
				...
				if (propsSet == (_repository)) {
					...
					buf.append("/snapshots");
					buf.append("/");
					SimpleEndpoint.pathEncode(request.repository.stream().map(v -> v).filter(Objects::nonNull)
			...
			request -> {
				...
				if (propsSet == (_repository)) {
					params.put("repository", request.repository.stream().map(v -> v).filter(Objects::nonNull)

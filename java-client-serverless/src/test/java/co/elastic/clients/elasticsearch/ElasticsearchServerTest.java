...
public class ElasticsearchServerTest extends Assertions {

    static ElasticsearchClient client;

    @BeforeAll
    public static void setup() throws IOException {
        File propFile = new File(new File(System.getProperty("user.home")), ".elastic/serverless-test.properties");
        if (!propFile.exists()) {
            fail("Missing file ~/.elastic/serverless-test.properties");
        }
        ...
        String apiKey = properties.getProperty("apiKey").toString();

        RestClient restClient = RestClient
            .builder(HttpHost.create(url))
            .setHttpClientConfigCallback(c -> c
                .setDefaultHeaders(Arrays.asList(new BasicHeader(HttpHeaders.AUTHORIZATION, "ApiKey " + apiKey)))
            )
            .build();
        ...
        client.indices().delete(d -> d.index("my-index", "ingest-test", "products", "testindex", "foo").ignoreUnavailable(true));
    }
...
// Default lifecycle policy has no name
//    @Test
...
//        );
//
//        System.out.println(resp.valueBody().toJson());
